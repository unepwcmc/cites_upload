module ReportsHelper
  ...
  def link_to_add_fields(name, f, association, file_type, hidden_button)
    ...
    "#{link_to_function(name, "add_fields(this, '#{association}', '#{escape_javascript(fields)}')", :class => "btn add_file #{hidden_button ? "hide" : ""}")}<p class='clear' />".html_safe
  end
end
